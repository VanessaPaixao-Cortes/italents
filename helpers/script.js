...
function createProductItemElement({ id: sku, nome: name, url_imagem: image }) {
...
function createCartItemElement({ id: sku, nome: name, preco: salePrice }) {
...
window.onload = () => {
    const addChildrenSectionItems = async () => {
        ...
        const requisitionObject = await fetchProducts('pets');
        const loading = document.querySelector('.loading');
